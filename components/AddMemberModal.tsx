...
import { X, User, Phone, Mail, Calendar, MapPin, Key, Save, Loader2 } from 'lucide-react';
...
export default function AddMemberModal({ isOpen, onClose }: AddMemberModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[AddMemberModal] Register error:', err);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="w-full max-w-2xl bg-white rounded-2xl border shadow-xl p-6 md:p-8 animate-fade-in-up max-h-[90vh] flex flex-col">
        ...
        {/* Form content (scrollable on smaller viewports) */}
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto space-y-6 pr-1 text-sm font-medium">
            ...
            <div className="md:col-span-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border bg-background py-2.5 pl-10 pr-4 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
                />
              </div>
            </div>
          ...
          {/* Action Footer Buttons inside the scroll boundaries for a stable layout */}
          <div className="flex gap-4 justify-end border-t pt-5 mt-6 shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl border text-sm font-semibold hover:bg-muted/40 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 rounded-xl bg-primary py-2.5 px-6 text-sm font-bold text-primary-foreground hover:bg-primary/95 transition-all shadow-sm disabled:opacity-50 cursor-pointer"
            >
              {loading ? (
